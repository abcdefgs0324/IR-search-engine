...
def OneWordSearch(query):
	positionalFile = codecs.open('C:\\Appserv\\www\\IR\\PositionalList.txt', 'r')
		...
		if query == thisIndex:
			...
			while line.find('#') != -1:
				...
				if line.find('#') != -1 and line.find('-') != -1 and line.find('-') < line.find('#'):
					...
					docFirstIndex.append(int(line[line.find('&')+1:line.find('@')]))
			for i in range(0,len(docIDArr)-1):
				for j in range(0,len(docIDArr)-1-i):
					if docCountArr[j] < docCountArr[j+1]:
						...
						docFirstIndex[j] ^= docFirstIndex[j+1]
			for i in range(0,len(docIDArr)):
				ansStr += "@" + str(docIDArr[i]) #
				# ansStr += "@" + str(docIDArr[i]) + "-" + str(docFirstIndex[i])
			ansStr += "$" + str(sum(docCountArr))
			break

	if inList == 0:
		for docNum in range(1,1201):
			filePath = 'C:\\Appserv\\www\\IR\\articles_transfer\\' + str(docNum) + '.txt'
			...
			# ansStr += "@" + str(docNum) + "-" + str(docLine.find(query))
			docFile.close()
		...
		if len(docIDArr) == 0:
			...
			for i in range(0,len(docIDArr)):
				ansStr += "@" + str(docIDArr[i]) #
				# filePath = 'articles_transfer\\' + str(docIDArr[i]) + '.txt'
				# docFile = open(filePath, 'r')
				# docAllLine = docFile.readlines()
				# docLine = ""
				# for s in docAllLine:
				# 	docLine += s.strip()
				# ansStr += "@" + str(docIDArr[i]) + "-" + str(docLine.find(query))
				# docFile.close()
			ansStr += "$" + str(sum(docCountArr))
	return ansStr
...
def MulWordSearch(query):
	...
	for i in range(0,len(allDocID)):
		for j in range(0,len(allDocID)-1-i):
...
def proximitySearch(query):
	...
	ansQueryID = []
	for i in range(0,len(bothQueryDocID)):
	...
	ansQueryCount = []
	for i in range(0,len(ansQueryID)):
		ansQueryCount.append(allDocFreCount[allDocID.index(ansQueryID[i])])
	for i in range(0,len(ansQueryID)):
		for j in range(0,len(ansQueryID)-1-i):
...
def BooleanSearch(query):
	...
	while searchArr.count("not") > 0:
		num = 0
		for k in range(0,searchArr.index("not")):
	...
	while searchArr.count("and") > 0:
		num = 0
		for k in range(0,searchArr.index("and")):
			if searchArr[k] != "or":
				num += 1
		andArr.append(num)
	...
	while searchArr.count("or") > 0:
		num = 0
		for k in range(0,searchArr.index("or")):
			num += 1
		orArr.append(num)
		searchArr.remove("or")
	...
	searchTDArr = []
	for i in range(0,len(searchArr)):
		TDfile = codecs.open('C:\\Appserv\\www\\IR\\TDmatrix.txt', 'r')
	...
	ansQueryID = []
	ansQueryCount = []
	for i in range(0,len(searchTDArr)):
		if searchTDArr[i] != "":
			thisLine = searchTDArr[i][searchTDArr[i].find(',')+1:].strip() + ","
			for docID in range(1,1201):
			...
			break
	#
	for i in range(0,len(ansQueryID)-1):
		for j in range(0,len(ansQueryID)-1-i):
			if ansQueryCount[j] < ansQueryCount[j+1]:
				ansQueryCount[j] ^= ansQueryCount[j+1]
				ansQueryCount[j+1] ^= ansQueryCount[j]
...
if __name__ == '__main__':
	queryFile = codecs.open('C:\\Appserv\\www\\IR\\query.txt', 'r')
	queryLine = queryFile.readline()
	queryLine = queryLine.strip()
	queryFile.close()
	result = open('C:\\Appserv\\www\\output.txt' , 'w');
	queryResult = ""
